...
static const wchar_t* g_PropNames[] = {
	L"Milliseconds", L"RegexTamplate", L"UserString", L"TypeTamplate"
};
static const wchar_t* g_MethodNames[] = {
	L"Sleep", L"ToExpression", L"Validate"
};
...
bool CAddInNative::GetPropVal(const long lPropNum, tVariant* pvarPropVal)
{
	...
	case ePropMilliseconds:
		...
		TV_UI4(pvarPropVal) = m_uiMilliseconds;
		break;
	case ePropRegexTamplate:
		string_to_p(m_uiTamplate, pvarPropVal);
	default:
	case ePropUserString:
		string_to_p(m_uiUserString, pvarPropVal);
		return false;
	}
...
bool CAddInNative::SetPropVal(const long lPropNum, tVariant* varPropVal)
{
	...
	case ePropMilliseconds:
		if (TV_VT(varPropVal) != VTYPE_UI4)
			return false;
		m_uiMilliseconds = TV_UI4(varPropVal);
		break;
	case ePropRegexTamplate:
...
VOID CALLBACK MyTimerProc(PVOID lpParam, BOOLEAN TimerOrWaitFired)
{
	if (!pAsyncEvent)
		return;
	DWORD dwTime = 0;
	wchar_t* who = L"ComponentNative", * what = L"Sleep";
...
bool CAddInNative::string_to_p(std::string& str, tVariant* val)
{
